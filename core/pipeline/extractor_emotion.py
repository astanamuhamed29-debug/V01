...
async def extract(user_id: str, text: str, intent: str, person_id: str) -> tuple[list[Node], list[Edge]]:
    ...
    lowered = text.lower()
    if not re.search(r"(боюсь|страшно|страх|тревож|рад|радость|злюсь|злость|груст|печал|стыд|устал|вина|обид|ступор|чувствую|ненавижу\s+себя|презираю\s+себя)", lowered):
        return nodes, edges
